...
'''
Using the "Resampling Wheel" method to use the weights of each particle and
create a new list with the most likely particles
'''
p3 = []
index = random.randrange(0, N)
beta = 0.0
wmax = max(w) # there is a handy function to get maximum value from a list!

for i in range(N):
    beta += random.random() * 2 * wmax
    while w[index] < beta:
        beta -= w[index]
        index = (index + 1) % N
    p3.append(p[index])
